...
// ── JD Supra RSS ──────────────────────────────────────────────────────────────
// Category-level feeds — broad, so we keyword-filter client side.
const JDS_FEEDS = [
  {
...
async function fetchJDSupraFeed(feedDef) {
  try {
    ...
    for (const item of rawItems) {
      ...
      // Keyword filter — only keep if title or description mentions a contaminant
      if (!matchesKeyword(title) && !matchesKeyword(desc)) continue;

      // Detect which contaminant tag best fits
      ...
      // Extract author/firm if available
      ...
      if (out.length >= 15) break; // cap per feed before filtering
    }
    return out;
  } catch (e) {
...
async function main() {
  ...
  // Merge, deduplicate by URL, sort newest first, cap at MAX_ITEMS
